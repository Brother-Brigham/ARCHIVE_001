
- Chrom x Sumia ✔️
	- Lucina@Sniper
		- Galeforce (Dark Flier) {Sumia}
		- Aether (Great Lord) {Chrom}
		- Luna (Great Knight) {Lucina}
		- Bowfaire (or Speed +2) (Sniper) {Lucina}
		- Dual Strike+ (Lord) {Lucina}

	- Cynthia@Sniper
		- Galeforce (Dark Flier) {Sumia}
		- Aether (Great Lord) {Chrom}
		- Luna (Great Knight) {Cynthia}
		- Speed +2 (Pegasus Knight) {Cynthia}
		- Defender (Paladin) {Cynthia}

- Lissa x Ricken✔️
	- Owain@Sage
		- Galeforce (Dark Flier) {Lissa}
		- Tomefaire (Sage) {Ricken}
		- Astra (Swordmaster) {Owain}
		- Luna (Great Knight) {Owain}
		- Magic +2 or Defender (Mage or Paladin) {Owain}

- Olivia x Libra
	- Inigo@Sage
		- Galeforce (Dark Flier) {Olivia}
		- Tomefaire (Sage) {Libra}
		- Vengeance (Sorcerer) {Inigo}
		- Vantage (Myrmidon) {Inigo}
		- Magic +2 (Mage) {Inigo}

- Sully x Donnel
	- Kjelle@Hero
		- Galeforce (Dark Flier) {Kjelle}
		- Swordfaire (Swordmaster) {Kjelle}
		- Luna (Great Knight) {Sully}
		- Defender (Paladin) {Kjelle}
		- Speed +2 (Pegasus Knight) {Kjelle}

- Miriel x Gregor
	- Laurent@Hero (or anything I need)
		- Axefaire (Berserker) {Gregor}
		- Anathema (Dark Mage) {Miriel}
		- Hex (Dark Mage) {Laurent}
		- Tomebreaker (Sorcerer) {Laurent}
		- Axebreaker (Hero) {Laurent}

- Maribelle x Lon’qu
	- Brady@Sage
		- Galeforce (Dark Flier) {Maribelle}
		- Tomefaire (Sage) {Brady}
		- Astra (Swordmaster) {Lon’qu}
		- Luna (Great Knight) {Brady}
		- Defender (Paladin) {Brady}

- Cordelia x Virion
	- (Hatsune Miku) Severa@Sniper
		- Galeforce (Dark Flier) {Cordelia}
		- Bowfaire (Sniper) {Virion}
		- Vengeance (Sorcerer) {Severa}
		- Anathema (Dark Mage) {Severa}
		- Speed +2 (Pegasus Knight) {Severa}

- Tharja x Gaius
	- Noire@Assassin
		- Galeforce (Dark Flier) {Noire}
		- Bowfaire (Sniper) {Tharja}
		- Astra (Swordmaster) {Gaius}
		- Luna (Great Knight) {Noire}
		- Speed +2 (Pegasus Knight) {Noire}

- Nowi x Vaike
	- Nah@Hero
		- Axefaire (Berserker) {Vaike}
		- Strength +2 (Wyvern Rider) {Nah}
		- Swordbreaker (Wyvern Lord) {Nowi}
		- Deliverer (Griffon Rider) {Nah}
		- Move +1 (Thief) {Nah}

- Cherche x Henry
	- Gerome@Berserker
		- Axefaire (Berserker) {Gerome}
		- Strength +2 (Wyvern Rider) {Gerome}
		- Axebreaker (Hero) {Gerome}
		- Swordbreaker (Wyvern Lord) {Cherche} 
		- Tomebreaker (Sorcerer) {Henry}

- Panne x Stahl
	- Yarne@Berserker
		- Axefaire (Berserker) {Yarne}
		- Hit Rate +20 (Sniper) {Yarne}
		- Prescience (Archer) {Stahl}
		- Aggressor (Dread Fighter) {Dread Fighter item}
		- Limit Breaker (Limit Breaker item) {Limit Breaker item}

- Robin (M) x Noire
	- Morgan (F)@Sniper
		- Galeforce (Dark Flier) {Noire}
		- Bowfaire (Sniper) {Morgan}
		- Vengeance (Sorcerer) {Robin}
		- Vantage (Myrmidon) {Morgan}
		- Speed +2 (Pegasus Knight) {Morgan}